...
import { useRoqClient, useTroubleshootFindFirst } from 'lib/roq';
...
function TroubleshootEditPage() {
  ...
  const { data, error, isLoading, mutate } = useTroubleshootFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: TroubleshootInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.troubleshoot.update({
        data: values as RoqTypes.troubleshoot,
        where: {
          id,
        },
      });

...
import { useRoqPlatformUserProfiles, useTroubleshootCount, useWifiAccessCount, useCustomerCount } from 'lib/roq';

import { WifiAccessListPage as ShowCaseList } from 'pages/wifi-accesses';

function DashboardPage() {
  const { session } = useSession();

  const troubleshootStat = useTroubleshootCount();

  const wifiAccessStat = useWifiAccessCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Troubleshoots',
      result: troubleshootStat?.data,
      isLoading: troubleshootStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Wifi Accesses',
      result: wifiAccessStat?.data,
      isLoading: wifiAccessStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Customers',
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Wifi Accesses
                </Text>
                <NextLink href={`wifi-accesses`} passHref legacyBehavior>
                  <Button
